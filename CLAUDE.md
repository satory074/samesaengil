...
- `src/pages/index.astro`: **サイトはこの1ページだけ**。年/月/日セレクトのフォーム（SSR）＋空の `#result`、末尾で `boot()` を起動。`components/Layout.astro` が head/OGP/フッタ（`title`/`description`/`canonical`/`image` の props）。画面下端に固定のバックリンクバー（→ satory074.com/apps。正準仕様は basecamp/docs/app-backlink.md、marker クラス `app-backlink-bar` は横断監査用に変更不可）。
