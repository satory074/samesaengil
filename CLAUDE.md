...
生年月日を入れると、その誕生日にまつわる情報（同じ誕生日の有名人＝顔写真つき・フィクションキャラ・記念日・**同じ誕生日に発売されたゲームソフト（生まれた日ちょうどの分は⭐）**・**同じ誕生日に投稿されたニコニコ動画＝ミリオン(100万再生)達成のみ・サムネつき（生まれた日ちょうどの分は⭐）**・**その年の出来事＋生まれた週のオリコン1位（Spotify リンク・ジャケットつき、その年の週間1位一覧も）**・**同じ学年の有名人＝芸能/スポーツ/音楽…のカテゴリ別**・星座/誕生石/誕生花/年齢/干支/和暦・**曜日/月齢/生誕日数/数秘/九星**）が出てくる若者向け静的サイト。Astro 5 + Tailwind v4 + TypeScript、GitHub Pages（Actions デプロイ）。公開: https://satory074.github.io/samesaengil/
...
npx tsx scripts/smoketest.ts   # 純関数（almanac / share / year / peers / photos / initials）
...
NICO_ONLY=1 npm run aggregate              # Wikipedia を叩かず「ミリオン動画」だけ差し替え（数秒）
...
npm run import:nico            # スナップショット検索API v2 → src/data/nicovideos.json（ミリオン動画。週次 CI でも実行、~2分）
...
- `NICO_MIN_GAP_MS` … **ミリオン動画の取込のみ**（既定 1200ms＝約50 req/分）。キーは不要（スナップショット検索API v2 は無認証）。
...
- **ランキング**（`aggregate.ts` の `buildPeopleAndAnimals`）: 名前・肩書きは日本語リスト由来、写真は ja pageimages。**正規化タイトルで重複排除**し、並びは **閲覧数(fame) 降順 → 写真あり → 生年新しい順**（＝日本でよく見られている人が上位）。**全件**を JSON に保存し、表示側で先頭30件＋「もっと見る」遅延描画。動物は別配列 `animals`（データは分けたまま。表示側では有名人グリッドに統合される——render.ts の項参照）。
...
- **発売されたゲームソフト** `sources/jawikiGameList.ts`（純関数パーサ）＋`sources/gameSources.ts`（機種→記事名）＋`sources/steamStore.ts`（PC）＋取込 `importGames.ts` → コミット済み `src/data/games.json`（4.6万本・6.2MB。うち Steam 271本）。kinenbi と同じ「取込→静的 JSON→aggregate は読むだけ」パターン（**新作を拾うため週次 CI でも実行**）。
  ...
  - 取込後の反映は `npm run rank:games`（新しいタイトルの閲覧数を state に足す）→ `npm run import:covers` → `GAMES_ONLY=1 npm run aggregate`（数秒）。`rank:games` を先に回すのは `CHARS_ONLY` と同じ理由（`import:covers` の人気順にもこの閲覧数を使う）。
- **投稿されたニコニコ動画（ミリオン達成）** `sources/nicoSnapshot.ts`＋取込 `importNicovideos.ts` → コミット済み `src/data/nicovideos.json`（**7,161本・約1.1MB**。1日あたり中央値20本）。games / kinenbi と同じ「取込→静的 JSON→aggregate は読むだけ」パターン（**新たにミリオンへ到達した動画を拾うため週次 CI でも実行**）。
  - **ソースはニコニコ動画「スナップショット検索API v2」**（`snapshot.search.nicovideo.jp`、**キー不要**）。`q=` 空のキーワードなし検索に `filters[viewCounter][gte]=1000000` を掛けると、全ジャンル横断の「ミリオン動画」がそのまま取れる。規約は**非営利利用のみ**・User-Agent にサービス名（`lib/util.ts` の `USER_AGENT` が該当）。`_sort` は**必須**（省くと 400）。
  - **年ごとにページング**（`filters[startTime]` で年を切り、`_offset` を 100 ずつ）。理由は 2 つ: 1回の offset を小さく保てること、**失敗した年だけ前回値に戻せる**こと（importGames の「機種ごとに前回値フォールバック」と同じ単位）。全年で ~92 リクエスト・約2分。
  - **`_sort=+startTime`（投稿日時の昇順）で引く**。投稿日時は不変なのでページ境界が動かない。`-viewCounter` だと取得中に順位が動いて取りこぼす。
  - **`startTime` は `Date` に通さない**（`sources/nicoSnapshot.ts` の `ymdOfStartTime`）。値は `2009-10-27T03:13:22+09:00` の JST で、UTC ランナー（GitHub Actions）で `Date` に通すと **00:00〜09:00 JST 投稿ぶん（約12%）が前日にズレる**——ローカル（JST）では再現しないので気付けない。文字列から正規表現で切り出す。
  - **再生数は「万」単位で丸めて持つ**（`NicoVideo.man`）。表示が「3143万再生」で生値を要さないのに、生値だと週次 cron のたびに全7,161行が git の差分になるため。
  - **サムネは URL ではなくトークンだけ持つ**（`NicoVideo.thumb`、`Game.cover` と同じ規範）。旧形式 `.../8628149/8628149` と新形式 `.../43708803/43708803.68284955` の 2 系統があり、**新形式はサフィックスを落とすと 404**＝ID から導出できない。既定形（ID の数字部）と同じときだけ省略する。`so` 系はディレクトリ番号が ID と違うので、トークン側からディレクトリを取る（`src/lib/nicovideo.ts` の `nicoThumbUrl`）。
  - **並びは再生数の多い順 → 年の新しい順**。ゲームやキャラと違い `resolveWorkFame`／`state.json` を**使わない**——並べ替えに使う人気（再生数）がデータそのものに入っているので `buildNicoMap` は純関数。
  - ニコニコ動画のサービス開始は **2006年12月**（ミリオン到達の最古は 2007-03-06＝(γ)サービス開始日）。2006年より前生まれには ⭐ が構造的に出ないので、credit 行にその旨を書いている。
  - 取込後の反映は `NICO_ONLY=1 npm run aggregate`（数秒）。**`src/data/nicovideos.json` を `[]` にして同じコマンドを回せばセクションごと即撤去できる**（AniList と同じ脱出口）。
...
- `src/pages/index.astro`: **サイトはこの1ページだけ**。年/月/日セレクトのフォーム（SSR）＋空の `#result`、末尾で `boot()` を起動。`components/Layout.astro` が head/OGP/フッタ（`title`/`description`/`canonical`/`image` の props）。画面下端に固定のバックリンクバー（→ satory074.com/apps。正準仕様は basecamp/docs/app-backlink.md、marker クラス `app-backlink-bar` は横断監査用に変更不可）。フォームに **📅本日ボタン**（`data-action="set-today"`・`type="button"`）、`#result` の**直後**に追従ボタン `.float-nav`（トップへ/たたむ。CSS の `#result:empty + .float-nav` で診断前は非表示＝隣接セレクタなので位置が必須。バックリンクバー 48px の上・z-index 30）、末尾に「？」一覧オーバーレイの器 `[data-help-overlay]`。**これらは domtest の `appHtml()` と二重定義**——新要素は index.astro と scripts/domtest.ts の両方に足すこと（片方だけだと dev では動くのにテストが落ちる/その逆）。
...
- `src/app/main.ts` の `boot(root)`: クロージャ状態 ＋ `data-action` 委譲。フロー = 入力読取 → `isValidDate` 検証 → **per-day・per-year・per-学年 を `Promise.all` で並行 fetch**（学年は早生まれだと暦年の1つ前のファイルになるので別途引く。同じ年ならファイルを共用して余計な fetch をしない） → `almanac.ts` で暦を計算 → `render.ts` で `#result.innerHTML` を組み立て → `history.replaceState` で `?d=YYYY-MM-DD` 同期。ロード時に `?d=` があれば即描画。`normalizeDay`/`normalizeYear` が古い JSON の欠損キーを既定値で補う（**新キー追加時は必ずここも**）。**不正日付はフォーム直下の `.form-error`（role=alert）に出し、前回の結果と URL は消さない**（以前は `#result` を上書きしていて画面と `?d=` が食い違った）。**フォーム送信時だけ** `#result` へ `scrollIntoView`（`?d=` 付きロード時は飛ばさない。reduced-motion なら behavior:auto）。**前回入力の復元**: 診断成功時（検証通過後）に `src/app/storage.ts` が localStorage（キー `samesaengil:lastInput`、値は `encodeDate` の "YYYY-MM-DD"）へ保存し、`?d=` の無いロード時にフォームへ**入力のみ**復元（診断はしない。`?d=` があればそちら優先で従来どおり即描画）。localStorage は private mode で**アクセス自体が** throw するので読み書きとも try/catch＋`globalThis.localStorage?.`。追加の data-action: `set-today`（📅本日＝今日をフォームに入れるだけ・診断しない・`type="button"` 必須）／`scroll-top`（トップへ。reduced-motion 対応＋jsdom 用 try/catch）／`collapse-all`（`#result` 内の `details[open]` を**全部**閉じる——`.section` に限定しないのでネストの `.chart-list` も閉じる）。暦項目の「？」一覧は `src/app/help.ts` の `wireHelp`（データは almanac の export 表・HTML は開くたび組み立て・`dialog.showModal` 不使用＝jsdom で動かすため。オーバーレイの器は index.astro の静的マークアップで、開閉は `hidden` 属性——CSS の `.help-overlay[hidden]{display:none}` が無いと `display:grid` が勝つので必須）。
- `src/app/render.ts`: セクション別の HTML 文字列ビルダ（`esc()` で全データをエスケープ）。`resultHtml` が**唯一のセクション順序定義**（順は ✨プロフィール→🔮小ネタ→🎂生まれた年→📅何の日→🎤有名人→🦸キャラ→🎮ゲーム→📺ニコニコ動画→🎓同じ学年→シェアボタン）。**全セクションは `<details class="section">` の折りたたみで、初期に開くのは ✨プロフィールと🔮小ネタの2つだけ**（シェアボタン `.share-row` だけはセクションに入れず末尾に常時表示——2026-09 ユーザー要望で「シェアして盛り上がる」見出しを撤去）（`section()` ヘルパー1箇所で生成、`open` 引数で指定。`class="section"` は more.ts の `.closest(".section")` が依存するので変更不可。domtest も `.section` クラスで引く——`<section>` タグではないため）。診断ごとに `#result` は総入れ替えなので開閉状態は毎回リセット。🎂 セクション内に**その年の週間1位ぜんぶの一覧**（`chartListHtml`＝ネスト details）。誕生週のハイライトは **`w === song` の参照比較**——`songForBirthday` は chartWeeks の要素参照か prevYearLast を返すので、month/day 比較だと年始生まれ（song=前年末週）で当年の同月日週を誤ハイライトする。参照比較なら prevYearLast は一覧に無い＝自然にハイライト無し。ジャケットは有名人写真と同じ「プレースホルダ背面＋img 重ね＋onerror で戻る」流儀（`.cw-art`/🎵）。有名人カードは **イニシャルを背面に置き写真を被せる**方式（`.thumb[data-initials]` ＋ `img.photo onerror="this.remove()"`）＝写真が無い/失敗してもイニシャルが出る。**写真なしは例外ではなく一覧の 3〜4 割**（上記の構造的な理由）なので、プレースホルダは「主要な見た目」として作る: イニシャルは 2 文字（`今田美桜`→`今田`、`ゲオルク・オーム`→`ゲオ`）、`.thumb[data-cat]`（肩書きのカテゴリ＝`peers.ts` の `categorize`）で色相を出し分けて「読み込み失敗の灰色」ではなく「分類された色チップ」に見せる。キャラは1日 数百〜千件になりうるため（例 7/7 で ~1900 件）、有名人と同じく **先頭 `CHARS_VISIBLE`(=40) 件＋「もっと見る」遅延描画**。キャラ一覧は**作品ごとのグループ見出し**（`charRows`。作品人気順ソートで同一作品が隣接している前提。チップ側の作品名は省き、「もっと見る」境界では直前の作品名を引き継いで見出しの重複を防ぐ）。記念日セクションの見出しは**「M月D日は何の日」**（「今日」ではない）で、位置は「生まれた年」の直後（数百件のキャラ一覧の下だと辿り着けないため）。**🎤有名人は動物・名馬も含め1グリッドに完全統合**（2026-09 ユーザー要望でサブブロック＝K-POP/VTuber/「有名人ぜんぶ」/動物 の h3 区分けを全廃。`src/lib/oshi.ts` も削除）。統合は `render.ts` の `allBirthdayPeople(people, animals)`＝fame 降順の安定ソートマージ（全日 JSON で fame 欠落 0 を確認済み。オグリキャップ級は人間の上位に自然に混ざる）。**「もっと見る」用の main.ts の `lastPeople` も必ずこの関数を通した同じ配列にする**（peers.ts の exact と同じ「render と main で同じ切り方を再現」規範）。件数バッジは統合後の全件数（＝従来の人＋動物の合算と同値）。**`data-people-grid` は主グリッドだけに付ける**（more.ts が `.closest(".section")` 内で一意に引いて「もっと見る」を追記する前提）。⭐「生まれた日ちょうどに発売」を先頭ブロックに全件出し、残りを人気順で先頭30本＋「もっと見る」。各行の先頭に**ジャケット**（`.g-art` の器を全行に出し、あれば `.g-img` を重ねる＝`.cw-art`/`.cw-img` と同じ流儀。無い行は 🎮 のまま高さが揃う）（`.game-block.exact` は `.year-people-block.exact` と同じ「色で立てる」流儀）。**誕生日と発売日がピタリ一致する確率は年代により 2〜5 割**なので ⭐ が出ない日も多く、月日一覧の方が本体（1日あたり中央値100本前後）。📺ニコニコ動画（`nicoHtml`）は 🎮ゲームと**同型**（⭐ ブロック＋月日一覧・先頭30本＋「もっと見る」・`.n-art` の器を全行に出して `.n-img` を重ねる）。違いは行の中身: サムネが**横長 48×36**（動画のサムネは 130×100。縦長のゲームジャケと並んでもシルエットで区別がつく）、その右は**タイトル（2行でクランプ）＋「年・再生数」のメタ行**の縦積み、行は必ず `<a>`（全動画に watch URL がある）。**縦積みにしているのは折り返し対策**——動画タイトルは平均60バイトで【】つきと長く、ゲームと同じ1行構成にするとスマホ幅（実測 358px のコンテナ）でタイトル列に 92px しか残らず **1行が 210px 高**になった（縦積み＋2行クランプで 80px に収まる）。クランプで隠れた分は `a[title]` のホバーで読める。
- `src/app/more.ts`: 「もっと見る」の click 委譲。描画済みの全件配列から残りを `insertAdjacentHTML` で足す（初期 DOM を軽く保つため、有名人30件・キャラ40件・ゲーム30本・動画30本・同い年はカテゴリごと12件だけ先に描く）。同い年だけはカテゴリ別にグリッドが分かれるので、ボタンの `data-cat` と `[data-year-grid="<cat>"]` で対応づける。
- `src/app/share.ts`: `?d=` の encode/decode・`isValidDate`/`daysInMonth`/`isLeap` の純関数（DOM 非依存・テスト対象）。
- `src/lib/almanac.ts`: 星座/誕生石/誕生花/干支/和暦/世代/年齢 ＋ **ユリウス通日(JDN)ベース**の曜日/月齢/生誕日数/キリ番記念日/数秘ライフパス/九星の純関数。**`Date` を内部で使わない**（`ageOf`/`daysLivedOf` は基準日を引数で受ける）＝テスト可能。**一覧表（ZODIAC/ETO/BIRTHSTONES/BIRTH_FLOWERS/GENERATIONS/MILESTONES/LIFE_PATH_LABELS/KYUSEI）は export**——「？」ボタン一覧（`app/help.ts`）の単一ソース。世代は `GENERATIONS` 表駆動（**from 降順が `generationOf` の前提**。invariant は smoketest が守る）。
- `src/lib/days.ts`: `allDays()`（366日の列挙）。`aggregate.ts` が全日を回すのに使う（`aggregateYears.ts` の逆引きも）。
- `src/lib/nicovideo.ts`: 「同じ誕生日に投稿されたニコニコ動画」の**再カット**（新ソース無し）＋ URL 組み立て。`exactNicoOf` / `withoutExactNico` は `games.ts` と同じ役割で、**`render.ts` と `main.ts` の両方で同じ切り方を再現する必要がある**のも同じ。`nicoWatchUrl`（ID→動画ページ）と `nicoThumbUrl`（トークン→サムネ。トークンの先頭がディレクトリ名）、`viewsLabel`（万単位の表示）もここ。
...
- `src/lib/peers.ts`: 「同じ学年の有名人」セクションの学年判定・分類（新ソース無し＝既存データの再カット）。
...
- **ライト固定・グラデーション不使用**（2026-08 ユーザー要望「全体的に暗い」「グラデにしない」）: ダークテーマは削除し `color-scheme: light`（globals.css と Layout.astro の meta の両方）。配色は Open Color ベースのソリッドのみ——地 `--bg #faf9f7`／カード白／ニュートラルグレー3段／アクセントはローズ `--accent #d6336c`・`--accent-strong #c2255c` の1色（旧 `--pink`/`--purple` は全廃。OG 画像 `src/lib/og.ts` と `public/favicon.svg` は globals.css と独立に同色をハードコードしているので色変更時は3箇所同期）。文字色・アクセントは白地で WCAG AA 4.5:1 以上を維持すること。なお Chrome の **force-dark フラグ**（`chrome://flags/#enable-force-dark`）有効環境は paint 層で強制されるため CSS から抑止不可（実装の問題ではない）。
...
**Gotcha**: 人物は日本語版由来なので肩書きは初めから日本語（旧・英語版マージ＋翻訳は廃止）。キャッシュ/失敗フォールバックは `src/data/state.json`（`pages`＋`views`）＋前回per-dayファイル。デザインは**ライト固定・ソリッドカラーのみ**（`color-scheme: light`。ダークテーマ・グラデーションは 2026-08 に全廃。Chrome の force-dark フラグ有効環境はCSSから抑止不可＝paint層で強制）。Tailwind v4 + Astro 型不一致は `astro.config.mjs` で `any` キャスト、`base: "/samesaengil"`。`src/app/*`・`src/lib/*` は tsx テストのため**相対import**（@エイリアス不可）。
