...
生年月日を入れると、その誕生日にまつわる情報（同じ誕生日の有名人＝顔写真つき・フィクションキャラ・記念日・**その年の出来事＋生まれた週のオリコン1位（Spotify リンクつき）**・星座/誕生石/誕生花/年齢/干支/和暦・**曜日/月齢/生誕日数/数秘/九星**）が出てくる若者向け静的サイト。Astro 5 + Tailwind v4 + TypeScript、GitHub Pages（Actions デプロイ）。公開: https://satory074.github.io/samesaengil/
...
npx tsx scripts/smoketest.ts   # 純関数（almanac / share / year / oshi）
...
- **パーサ破損の検知**: 「できごと0件」「1968年以降なのに週間1位0件」の年をログに列挙する。Wikipedia のテンプレ/節構成は編集されるので、サイレント破損はこれで気付く。Spotify も「新規解決/未収録/失敗」の件数をログに出す。
...
- `src/app/more.ts`: 「もっと見る」の click 委譲。描画済みの全件配列から残りを `insertAdjacentHTML` で足す（初期 DOM を軽く保つため、有名人30件・キャラ40件だけ先に描く）。
...
- `src/lib/days.ts`: `allDays()`（366日の列挙）。`aggregate.ts` が全日を回すのに使う。
