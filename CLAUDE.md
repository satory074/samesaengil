...
- **知名度（並び替え指標のみ）** `sources/wikidataEntities.ts`: ja の Q-ID を **`wbgetentities` Action API**（SPARQL ではない）で 50 件ずつ・3 並列補完 → sitelink 数(=fame)。**人物リスト自体は増やさず、fame は並び順にだけ使う**。
- **ランキング**（`aggregate.ts` の `buildPeopleAndAnimals`）: 名前・肩書きは日本語リスト由来、写真は ja pageimages。Q-ID（無ければ `name:year`）で重複排除し、並びは **fame 降順 → 写真あり → 生年新しい順**。**全件**を JSON に保存し、表示側で先頭30件＋「もっと見る」遅延描画。動物は別配列 `animals`。
...
ソース毎 `try/catch`、失敗時は**前回の per-day ファイル**へフォールバック（jawiki 誕生日が取れなかった日のみ people/animals を前回値に戻す）。横断キャッシュは `src/data/state.json`（`entities`: Q-ID→fame 等、`pages`: jawiki title→{qid,photo}（負キャッシュ込み））。再実行時は未キャッシュの title/Q-ID だけ取得。
